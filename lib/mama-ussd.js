...
function MamaUssd() {
        ...
        p.add_callback(function(result){
            if (result.response.status === "0000"){
                var credit = {
                    ...
                    reference: self.get_today(im).valueOf(),
    ...
    self.run_airtime_giveaway = function(){
        ...
        if(Math.floor((Math.random()*im.config.airtime_chance)+1)==1 || im.config.testing){
            // Winner so check threshold for week not hit
            var monday = self.get_monday(self.get_today(im)).toISOString().substring(0,10);
            // Creates a key like "2013-06-10_airtime"
            var metric_key = monday+"_airtime_committed";
            var p = im.api_request('kv.get', {
                key: metric_key
            });
            p.add_callback(function(result){
                if (result.value < im.config.airtime_max_per_week) {

...
function MamaUssd() {
    ...
    self.add_creator('register_all_smsoptin', function(state_name, im) {
        ...
        p.add_callback(function(contact) {
            return im.api_request('contacts.update_extras', {
                key: contact.key,
                fields: {
                    "optin_hiv": im.get_user_answer('register_all_hivinfo')
                }
            });
        });
    ...
    self.add_creator('register_all_thanksandstart', function(state_name, im) {
        ...
        //         var status = im.get_user_answer('register_all_1');
        //         var quiz_start_week = result.contact.extras.mama_child_dob
        var week = "5";  // needs updating with dynamic when above fixed ^^^
        var prefix = im.get_user_answer('register_all_1') == "pregnant" ? "prebirth" : "postbirth";

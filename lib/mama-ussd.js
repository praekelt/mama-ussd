...
function MamaUssd() {
    ...
    self.add_creator('initial_state', function(state_name, im) {
        ...
        p.add_callback(function(contact) {
            if (contact.extras.mama_registration_completed){
                var week = "5";  // needs updating with dynamic 
                var prefix = contact.extras.mama_status == "pregnant" ? "prebirth" : "postbirth";
                var question = im.config.quiz_data[prefix + "_" + week]['quiz_details']['questions'][im.config.quiz_data[prefix + "_" + week]["start"]];
                return self.make_initial_quiz_state(state_name, prefix + "_" + week, question);
            } else {
    ...
    self.add_creator('register_all_thanksandstart', function(state_name, im) {
        // Determin quiz start if they want that now
        // return im.get_user_answer('quiz_start')+"_"+choice.value+"_"+im.config.quiz_data[im.get_user_answer('quiz_start')+"_"+choice.value]["start"];
        ...
        // p.add_callback(function(result, state_name) {
        //     return result.contact;
        // });
        ...
        //     if (result.success) {
        //         var today = new Date();
        //         //self.calc_weeks(today,due_month)
        //         var status = im.get_user_answer('initial_state');
        //         var quiz_start_week = result.contact.extras.mama_child_dob
        var week = "5";  // needs updating with dynamic when above fixed ^^^
    ...
    self.on_config_read = function(event){
        for (var quiz_name in im.config.quiz_data){
            ...
            self.add_state(new EndState(
                quiz_name+"_end",
                im.config.quiz_data[quiz_name].end,
                "initial_state"
            ));
        }
    };
